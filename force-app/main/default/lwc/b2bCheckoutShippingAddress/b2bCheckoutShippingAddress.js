...
import { 
    ...
    mapRegionCodes,
    isEmptyObj
    } from './b2bCheckoutShippingAddressUtils';
import { WireStatus } from './b2bWireStatus';
import ModalCheckoutAddressForm from 'c/b2bModalCheckoutAddressForm';
import { mockedAddressData } from './b2bShippingAddressMock';

export default class B2bCheckoutShippingAddress extends useCheckoutComponent(LightningElement) {
    currentCommunityId;
    webstoreId;
    cpaRecordTypeId;
    ...
    /**
     * 
     * Get all cart items to check if we have shippable products
     * 
     */
    ...
    /**
     * 
     * Remove the existing Delivery Address from CartDeliveryGroup, if required.
     * 
     */
    async removeDeliveryAddress() {
        console.log('Working Delivery Address: ', this.workingAddress);
        const address = {
            addressId: this.workingAddress.addressId,
            street: '' ,
            city: '' ,
            region: '' ,
            postalCode: '',
            country: this.workingAddress.country
        };
        ...
        .then((result) => {
            console.log('Updated Delivery Address: ', result);
            // this.loadCheckout();
            
        })
        .catch((error) => {
            console.error('Error in Updating Delivery Address: ', error);
        });
    }
    ...
    async loadCheckout() {
        await loadCheckout({})
        .then((result) => {
            console.log('Load Checkout: ', result);
        })
        .catch((error) => {
    ...
    async handleAddressCommitEvent(event) {
        const addressForm = this.template.querySelector("c-b2b-address-form");
    ...
    /**
     * 
     * sets the Country Code picklist values for forms. The options are retrived from 
     * the picklist values in metadata for the field. 
     * 
     */
    ...
    /**
     * 
     * sets the region code picklist values for forms. The options are retrived from 
     * the picklist values in metadata for the field. 
     * 
     */
