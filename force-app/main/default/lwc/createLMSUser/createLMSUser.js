...
import createUser from "@salesforce/apex/ContactCourses_CTRL.createUser";
...
export default class CreateLMSUser extends LightningElement {
    @api recordId;
    @api objectApiName;
    ...
    @api invoke() {
        if (this.sxUserId) {
            showToast(true, 'SX User Id is already defined for this ' + this.objectApiName);
        } else if (!this.email) {
            showToast(true, 'Email is not defined for this ' + this.objectApiName);
        } else {
            ...
            createUser({ contactId: this.contactId })
                .then((result) => {
                    if (result) {
                        ...
                        showToast(false, 'New LMS User created for ' + this.name);
                    }
                })
                .catch((error) => {
